...
| `FIELD_ENCRYPTION_KEY` | The master KEK the record store wraps per-value DEKs under (base64-encoded 32-byte AES-256 key). Sourced through the [secrets manager](secrets) under its canonical secret name — from Doppler in production, from this process environment on the dev/bare path — never read into the data path directly. Required when `KURA_DATABASE_URL` is set. |
